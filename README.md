...
如果在 macOS 上打开应用时提示"应用已损坏"或"无法验证开发者"，这是因为应用未经过 Apple 公证。

**解决方法：打开终端，复制粘贴以下完整命令并执行**

如果应用安装在"应用程序"文件夹：

```bash
sudo xattr -cr /Applications/FTP.app && echo "✓ 已成功移除隔离属性，现在可以打开应用了"
```

如果应用在"下载"文件夹（下载后未移动）：

```bash
sudo xattr -cr ~/Downloads/FTP.app && echo "✓ 已成功移除隔离属性，现在可以打开应用了"
```

如果应用在桌面：

```bash
sudo xattr -cr ~/Desktop/FTP.app && echo "✓ 已成功移除隔离属性，现在可以打开应用了"
```

**说明：**
- 执行命令时需要输入 Mac 的登录密码（输入时不会显示，输入完按回车即可）
- 看到 "✓ 已成功移除隔离属性" 提示后，即可正常打开应用
- 此操作是安全的，只是移除了系统的隔离属性标记
